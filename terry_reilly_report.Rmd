...
- Each provider had their number of claims per 1K beneficiaries computed. 16 providers ended up with NA values for claims/1K due to either NAs in their antibiotic claims count or their beneficiaries count. NA values for these variables indicate counts less than 11.
- In the analyses below, two datasets were used. In one, all NA values for claims/1K were removed. In the other, NA values for claims/1K were imputed in the following way: NA values for the antibiotic claims count and the beneficiaries count were imputed as 10. The number of claims per 1K beneficiaries was then computed from these.
