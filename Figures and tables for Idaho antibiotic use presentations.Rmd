...
# Example breakout
# For this example, take the 10 most common values for prescriber type and show breakout for 2019 data only.
top_ten <- p %>% count(Prscrbr_Type) %>%
    arrange(desc(n)) %>%
    filter(row_number() %in% 1:10) %>%
    pull(Prscrbr_Type)

# Compute weighted values by provider type
prscrbr_type_2019 <- summarize_provider_data(
    providers_data = p %>%
        filter(year == 2019,
               Prscrbr_Type %in% top_ten),
    grouping_var = "Prscrbr_Type",
    type = "weighted") %>%
