...
county_yr_oe <- fit_gm_year %>%
    ...
    summarize(N = sum(!is.na(log_oe)),
              `Log(O/E)` = mean(log_oe, na.rm = TRUE), .groups = "drop")
...
counties_sf <- counties(state = "ID", cb = FALSE) %>%
    left_join(county_yr_oe %>%
                  filter(year == 2019) %>%
                  select(NAMELSAD = county, `Log(O/E)`, N),
              by = "NAMELSAD") %>%
    mutate(N = if_else(is.na(N), 0L, N),
           County = NAME)

# Figure. Version with points encoding number of providers.
library(plotly)
p <- counties_sf %>%
    ggplot(aes(fill = round(`Log(O/E)`, digits = 2),
               County = County,
               N = N,
               geometry = geometry)) +
    geom_sf(alpha = 0.8) +
    # stat_sf_coordinates(
    #     aes(size = n_providers, County = County, fill = mean),
    #     geom = "point", alpha = 0.2) +
    scale_fill_binned(name = "Log(Observed/Expected)",
                      type = "viridis") +
    # guides(fill = guide_colorsteps(title = "Log(Observed/Expected)",
    #                                order = 1),
    #        size = guide_legend(title = "N Providers",
    #                            order = 2)) +
...
ggplotly(p, tooltip = c("County", "Log(O/E)", "N"))
...
## General medicine providers in Plotly

```{r, eval = TRUE, echo = FALSE, error = FALSE, message = FALSE, warning = FALSE, fig.height = 8.5, fig.width = 8}

# Get US county shapes
url <- 'https://raw.githubusercontent.com/plotly/datasets/master/geojson-counties-fips.json'
counties <- rjson::fromJSON(file=url)

# Get table with US county FIPS
...
df2 <- tibble(fips = counties_sf$COUNTYFP,
              ...
              mean = counties_sf$`Log(O/E)`) %>%
    mutate(fips = paste0(state, fips)) %>%
    select(fips, county, state, mean)

# Join Idaho-specific info to df--the table with all US county FIPS
df3 <- df %>%
    left_join(df2, by = "fips") %>%
    mutate(hover = paste0(county, "<br>", round(mean, digits = 2)),
           trace2 = if_else(is.na(mean) & state == "16", 1L, NA_integer_))
...
plot_ly() %>%
    ...
    # Trace for counties with missing O/E
    add_trace(
        type = "choroplethmapbox",
        geojson = counties,
        locations = df3$fips,
        ...
        # Counties with missing values will be gray, but scale won't show
        colorscale = "Greys",
        showscale = FALSE,
        marker = list(line = list(width = 0),
                      opacity = 0.5)) %>%
    colorbar(title = "Log(Observed/Expected)",
             outlinewidth = 0,
             thickness = 35) %>%
    layout(mapbox = list(
...
# Figure. Version with labels showing number of providers and county names
counties_sf %>%
