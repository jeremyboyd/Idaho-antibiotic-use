...
Data are general medicine providers in 2019.

### claims_1k ~ Prscrbr_RUCA

Suggests inclusion of a squared RUCA term.
...
### claims_1k ~ Bene_Avg_Age

Suggests inclusion of a squared age term.

```{r, eval = TRUE, echo = FALSE, error = FALSE, message = FALSE, warning = FALSE}

# Definite squared term
gm2019 %>%
...
We remove terms one at a time and use Chi-squared tests to compare fits.
...
I start by removing cubed terms one at a time and comparing.

Models with and without Bene_Prop_White^3 are not statistically different. Therefore we are justified in removing Bene_Prop_White^3. This is the new baseline.
...
Models with and without Bene_Prop_Female^3 are not statistically different. Therefore we are justified in removing Bene_Prop_Female^3. This is the new baseline.
...
A model with Bene_Avg_Risk_Scre^3 fits significantly better than one without. So we keep Bene_Avg_Risk_Scre^3.
...
Now we remove squared terms one at a time and compare to the baseline model.

Models with and without Bene_Prop_White^2 are not statistically different. Therefore we are justified in removing Bene_Prop_White^2. This is the new baseline.
...
A model with Bene_Prop_Fem^2 fits significantly better than one without. So we keep Bene_Prop_Fem^2.
...
A model with Bene_Avg_Age^2 fits significantly better than one without. So we keep Bene_Avg_Age^2.
...
A model with Prscrbr_RUCA^2 fits significantly better than one without. So we keep Prscrbr_RUCA^2.
...
At this point I'm proposing the model below as the final model. I've simplified where possible by removing higher-order terms that don't improve model fit. All remaining terms are statistically significant.
...
I used the final model terms to fit a series of models---one per year---to general medicine data. Below I show residual and fit plots for each model. Residuals for each year appear to be randomly distributed. R-squared values range from 0.14 to 0.20.
...
The figure below indicates that coefficients for the final model predictors are remarkably stable across different years of data.
...
All maps show observed/expected antibiotic claims by Idaho county for 2019. Lighter colors represent more claims than expected after controlling for model covariates; darker colors represent fewer claims than expected. Gray indicates no data.
...
county_yr_oe <- fit_gm_year %>%
    ...
    summarize(N = sum(!is.na(oe)),
              mean_oe = mean(oe, na.rm = TRUE), .groups = "drop")
...
counties_sf <- counties(state = "ID", cb = FALSE) %>%
    left_join(county_yr_oe %>%
                  filter(Year == 2019) %>%
                  select(NAMELSAD = Prscrbr_County, mean_oe, N),
              by = "NAMELSAD") %>%
