...
by_type_2019 <- map(provider_types, function(type) {
    ...
    resid_plot <- bind_cols(fitted, residual) %>%
        ggplot(aes(x = fitted, y = residual)) +
        geom_point(alpha = 0.2) +
        labs(title = paste(year, type),
             ...
             y = "Residual\nlog(Antbtc_Tot_Clms)")
    fit_plot <- bind_cols(observed, fitted) %>%
...
# GETTING GOOD PREDICTION AND RESIDUALS FOR GAUSSIAN MODEL OF LOG(ANTBTC_TOT_CLMS), general medicine only. Follow up on this:
# 2. Use model comparison to add/subtract terms
# 3. Fit models for other provider types.
# 4. Fit models for other years.
...
## Antibiotic use by Idaho county

The figure below summarizes 2019 Medicare Part D antibiotic claims made by general medicine providers. Lighter colors represent more claims than expected after taking into account provider gender and rurality, and beneficiary health status, proportion of females, and proportion of whites. Gray indicates no data.
...
county_yr_oe <- fit_gm_year %>%
    ...
    summarize(mean = mean(log_oe, na.rm = TRUE), .groups = "drop")
...
# NEXT: need to loop to create figures by year, by provider type
# If we want a heatmap of antibiotic claims across all provider types then we need a model that includes provider type and interactions with other vars. Check residuals on this.
# Should age be in models?
