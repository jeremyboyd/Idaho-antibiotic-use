...
# Only general medicine providers from 2019
p2019.3 <- p2019.2 %>%
    filter(Std_Provider_Type == "General Medicine")
...
# Model
qp_gm1 <- glm(Antbtc_Tot_Clms ~ offset(log(Tot_Benes)) + 1 +
                        primary_RUCA +
                        Prscrbr_Gndr +
                        Bene_Avg_Risk_Scre +
                        percent_fem +
...
# Compute O/E ratios. Larger ratios mean we're underpredicting antibiotic counts. Smaller ratios mean we're overpredicting antibiotic counts. Karl's figure in PPT has zero as the midpoint rather than 1. More like log-odds.
# Note that I'm computing counts of observed / counts of expected. This should be the same as rates of observed / rates of expected (e.g., counts per 1K beneficiaries), because the number of beneficiaries will be the same and will cancel out. Might consider a weighted mean though when I summarize by county...
p2019.2 <- p2019.2 %>%
    mutate(oe_ratio = Antbtc_Tot_Clms / pred_qpois9,
           log_oe_ratio = log(oe_ratio))

# Visualize OE ratio
p2019.2 %>%
    mutate(Prscrbr_NPI = fct_reorder(Prscrbr_NPI, oe_ratio)) %>%
    ggplot(aes(x = Prscrbr_NPI, y = oe_ratio)) +
    geom_col()

# Visualize log(OE ratio). Providers with positive log(OE ratio) have higher
# empirical claims relative to model predictions.
p2019.2 %>%
    mutate(Prscrbr_NPI = fct_reorder(Prscrbr_NPI, log_oe_ratio)) %>%
    ggplot(aes(x = Prscrbr_NPI, y = log_oe_ratio)) +
    geom_col() +
    theme(panel.grid.major.x = element_blank(),
          axis.text.x = element_blank(),
          axis.ticks.x = element_blank()) +
    labs(title = "Log(observed/expected) antibiotic counts",
         subtitle = "2019 Idaho by-provider data, N = 3,321")

# Use tigris to get shapes of zip codes
library(tigris)

# Have a couple of zips starting with "60". Are those in Idaho? Check. All
# others start with "83".
idaho_zips <- p2019.2 %>%
    select(zip = Prscrbr_Zip5) %>%
    unique()

# Retrieve shapfiles for all zip codes starting with "83
zips_sf <- zctas(cb = TRUE, starts_with = "83", year = "2019",
                 class = "sf") %>%
    select(zip = ZCTA5CE10, geometry)

# Filter to just the ones we want
zips_sf <- zips_sf %>%
    inner_join(idaho_zips, by = "zip")



# Compute mean log_oe by county
county_OEs <- p2019.2 %>%
    group_by(county) %>%
    summarize(mean = mean(log_oe_ratio, na.rm = TRUE), .groups = "drop") %>%
    arrange(desc(mean))


# Shapes for Idaho counties
counties_sf <- counties(state = "ID", cb = FALSE) %>%
    left_join(county_OEs %>%
                  rename(NAMELSAD = county),
              by = "NAMELSAD") %>%
    mutate(NAME = str_wrap(NAME, 5))
...
# Requires that Google Maps Static API is enabled
library(ggmap)
library(ggrepel)
register_google(key = api_key)

# Idaho bounding box (left, bottom, right, top)
id_box <- c(left = -117.5, bottom = 41.7, right = -110.5, top = 49.2)

# For some reason I'm only able to get the stamen terrain map
basemap <- get_map(id_box, source = "stamen", maptype = "toner-lite")

    
# This looks pretty good.
# Missing data for Clark & Lewis counties
counties_sf %>%
    ggplot(aes(fill = mean, label = NAME)) +
    geom_sf() +
    geom_sf_text(size = 3) +
    scale_fill_continuous(name = "Log(Observed/Expected)",
                          type = "viridis") +
    ...
    labs(title = "2019 Medicare Part D antibiotic claims",
         subtitle = str_wrap("Lighter colors represents more claims than expected after taking into account provider type, gender, rurality, and beneficiary age, health status, proportion of females, and proportion of caucasians.", 100))
    


# Look at getting data for missing counties

# Some kind of writeup for Karl. Include residuals from lm versus quasipoisson.



.
p2019.2$resid_qpois9 <- residuals(qpois9, type = "pearson")[]

# Roughly normally distributed, but with a long right tail
p2019.2 %>%
    ggplot(aes(x = resid_qpois9)) +
    geom_density()

# Residuals versus fitted. Looks like more variance in residuals as fitted
# increase (fan shape).
p2019.2 %>%
    select(fitted = pred_qpois9, residual = resid_qpois9) %>%
    ggplot(aes(x = fitted, y = residual)) +
...
# QQ plot
qqnorm(p2019.2$resid_qpois9)
qqline(p2019.2$resid_qpois9)

# What do residuals look like for negative binomial?. Distribution has mode < 0
# and shorter tail. Less fan-like, more funnel-like. Worse qq-plot.
p2019.2$resid_nb9 <- residuals(nb9, type = "pearson")
p2019.2 %>%
    ggplot(aes(x = resid_nb9)) +
    geom_density()
p2019.2 %>%
    select(fitted = pred_nb9, residual = resid_nb9) %>%
    ggplot(aes(x = fitted, y = residual)) +
    geom_point(alpha = 0.2)
qqnorm(p2019.2$resid_nb9)
qqline(p2019.2$resid_nb9)

# What do residuals look like for a gaussian model?. Distribution of residuals
# seems to have a mode < 0 and very long tail. Definite fan shape, quite a bit more likely to miss low than high. High values
# off by hundreds in QQ plot.
lm9 <- lm(Antbtc_Tot_Clms ~ offset(log(Tot_Benes)) + 1 +
                        Prscrbr_RUCA +
                        I(Prscrbr_RUCA^2) +
                        Prscrbr_Gndr +
...
p2019.2$resid_lm9 <- residuals(lm9)
p2019.2 %>%
    ggplot(aes(x = resid_lm9)) +
    geom_density()
p2019.2 %>%
    select(fitted = pred_lm9, residual = resid_lm9) %>%
    ggplot(aes(x = fitted, y = residual)) +
    geom_point(alpha = 0.2)
qqnorm(p2019.2$resid_lm9)
qqline(p2019.2$resid_lm9)
...
# If we're really dropping all 0 claims then the strengths of the hurdle and zeroinfl models aren't needed as much. Neither model will even work without zeros in the data--throw errors complaining that minimum count isn't zero. For Antbtc_Tot_Clms we can't really get to zero, since the zeros that are in there are kind of suspect.

# So go back to poisson, quasi-poisson, and negative binomial
...
model1 = glm(cases ~ city + age, offset=lpop, family=poisson, data=eba1977)
summary(model1)
